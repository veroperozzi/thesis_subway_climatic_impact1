...
def calcular_nueva_acumulada(accumulada_guardada, actual_en_csv):
    if len(accumulada_guardada) == len(actual_en_csv):
        ...
        estacion = accumulada_guardada[3]
        pax_pagos = accumulada_guardada[4]
        pax_pases_pagos = accumulada_guardada[5]
        pax_freq = accumulada_guardada[6]
        total = accumulada_guardada[7]

        ## valores acumulados

        pax_pagos = pax_pagos + convertir_string_a_entero(actual_en_csv[4])
        pax_pases_pagos = pax_pases_pagos + convertir_string_a_entero(actual_en_csv[5])
        pax_freq = pax_freq + convertir_string_a_entero(actual_en_csv[6])
        total = total + convertir_string_a_entero(actual_en_csv[7])

        lista_acumulada_calculada = [fecha, linea, molin, estacion, pax_pagos, pax_pases_pagos, pax_freq,
                                     total]
    ...
    for fila_actual in csv_reader:
        if i == 0:
            csv_headers = [fila_actual[0], fila_actual[3], fila_actual[4], fila_actual[5], fila_actual[6],
                           fila_actual[7], fila_actual[8], fila_actual[9]]
        if i >= 1 and len(fila_actual[0].split('-', -1)) == 3:
            # hay que corregir la fecha , cada row la va  a tener como anio-mes-dia
            fecha = fila_actual[0]
            fecha_corregida = fecha.split('-', -1)
            actual_fecha = fecha_corregida[2] + "/" + fecha_corregida[1] + "/" + fecha_corregida[0]
            actual_linea = fila_actual[3]
            actual_molin = fila_actual[4]
            actual_estacion = fila_actual[5]
            actual_pax_pagos = convertir_string_a_entero(fila_actual[6])
            actual_pax_pases_pagos = convertir_string_a_entero(fila_actual[7])
            actual_pax_freq = convertir_string_a_entero(fila_actual[8])
            actual_total = convertir_string_a_entero(fila_actual[9])
            clave = actual_fecha + "_" + actual_linea + "_" + actual_molin + "_" + actual_estacion
            fila_actual_sin_horas = [actual_fecha, actual_linea, actual_molin, actual_estacion,
            ...
            if clave in diccionario:
                ...
                acc = [actual_fecha, actual_linea, actual_molin, actual_estacion, actual_pax_pagos,
...
def get_dia_de_la_semana(date_str):
    # para molinetes_2018 , las fechas son tipo : dd/mm/año
    import datetime
...
def eliminar_comillas_y_agregar_dia(list, dictionario):
    ...
    for l in list:
        ...
        ## ahora tenemos asi :  """02/01/2018""","""D""","""LINEA_D_9JULIO_S_TURN02""",6,"""9 DE JULIO""",597,1,7,605
        ## queremos dejarlo asi : "02/01/2018","D","LINEA_D_9JULIO_S_TURN02",6,"9 DE JULIO",597,1,7,605
        ...
        if i >= 1:
            ...
            # id_estacion = l[3]
            estacion = l[3].replace('"', '')
            pagos, pases_pagos, freq, total = l[4], l[5], l[6], l[7]

            precipitacion = 0
            if fecha in dictionario:
                precipitacion = dictionario[fecha]
            new_list_sin_comillas = [fecha, linea, molinete, estacion, pagos, pases_pagos, freq, total,
                                     dia_semana,
                                     precipitacion
...
csv_reader_m18 = open_file("molinetes_2018.csv")
headers, listas_comprimidas = csv_to_dict_of_list(csv_reader_m18)
...
write_to_csv(lista_sin_comillas, 'molinetes_2018_full.csv')
