...
def csv_to_dict_of_list(csv_reader):
    ...
    for fila_actual in csv_reader:
        ...
        if i == 0:
            csv_headers = [fila_actual[1], fila_actual[4], fila_actual[5], fila_actual[6], fila_actual[7],
                           fila_actual[8], fila_actual[9], fila_actual[10], fila_actual[11]]
        if i >= 1:
            actual_fecha = fila_actual[1]
            actual_linea = fila_actual[4]
            actual_molin = fila_actual[5]
            actual_id_estacion = convertir_string_a_entero(fila_actual[6])
            actual_estacion = fila_actual[7]
            actual_pax_pagos = convertir_string_a_entero(fila_actual[8])
            actual_pax_pases_pagos = convertir_string_a_entero(fila_actual[9])
            actual_pax_freq = convertir_string_a_entero(fila_actual[10])
            actual_total = convertir_string_a_entero(fila_actual[11])
            clave = actual_fecha + "_" + actual_linea + "_" + actual_molin + "_" + actual_estacion
            valor_actual = [actual_fecha, actual_linea, actual_molin, actual_id_estacion, actual_estacion,
                            actual_pax_pagos, actual_pax_pases_pagos, actual_pax_freq, actual_total]
            # che , tenes esta clave ? la viste antes ? te la di antes ?
            if clave in diccionario:
                # si , tomá . Te la dejo en lista_acumulada_guardada
                lista_acumulada_guardada = diccionario[clave]
                nueva_acumulada = calcular_nueva_acumulada(lista_acumulada_guardada, valor_actual)
                diccionario[clave] = nueva_acumulada
            else:
                acc = [actual_fecha, actual_linea, actual_molin, actual_id_estacion, actual_estacion,
                       actual_pax_pagos,
...
def get_dia_de_la_semana(date_str):
    # para molinetes_2017 , las fechas son tipo : dd/mm/año
    import datetime
...
def eliminar_comillas_y_agregar_dia(list, dictionario):
    ...
    for l in list:
        ...
        ## ahora tenemos asi :  """02/01/2017""","""D""","""LINEA_D_9JULIO_S_TURN02""",6,"""9 DE JULIO""",597,1,7,605
        ## queremos dejarlo asi : "02/01/2017","D","LINEA_D_9JULIO_S_TURN02",6,"9 DE JULIO",597,1,7,605
...
csv_reader_m17 = open_file("molinetes_2017.csv")
headers, listas_comprimidas = csv_to_dict_of_list(csv_reader_m17)
...
write_to_csv(lista_sin_comillas, 'molinetes_2017_full.csv')
